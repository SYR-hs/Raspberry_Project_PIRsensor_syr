...
try:
    ...
    while True:                                                        # 무한 루프 시작
        try:
            ...
            if sensorValue ==1:                                        # 센서값이 1이면 움직임이 감지된 것으로 판단하고 아래의 코드를 실행한다. 평상시에는 0.
                now = datetime.datetime.now()                          # 현재 날짜와 시간을 now 변수에 저장한다.
                print(now)                                             # 감지된(현재) 시각을 터미널에 출력
                fileName = now.strftime('%y-%m-%d %H:%M:%S')           # 시각을 파일명 형식 문자열로 변환하여 저장.
                picam2.capture_file(fileName + '.jpg')                 # 해당 파일명으로 jpg 사진 촬영 및 저장
                time.sleep(0.5)                                        # 0.5초 대기(연속촬영방지)

    except:                                                            # 촬영중 오류 발생시 무시하고 계속진행
        pass

except KeyboardInterrupt:                                              # 키보드 인터럽트(Ctrl+C) 발생시 루프 종료
